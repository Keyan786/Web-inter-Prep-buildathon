...
if os.environ.get('RENDER'):
    ...
    # Use absolute path to ensure we always hit the same consolidated database
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(os.getcwd(), 'instance', 'interview_prep.db')
...
def get_db_connection():
    ...
    try:
        if os.environ.get('RENDER'):
            ...
            # Local database connection using SQLite - using absolute path
            db_path = os.path.join(os.getcwd(), 'instance', 'interview_prep.db')
...
class ResumeData(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    personal_info = db.Column(db.Text)  # JSON: name, email, phone, linkedin, location
    professional_summary = db.Column(db.Text)
    work_experience = db.Column(db.Text)  # JSON array of work experiences
    ...
    template_style = db.Column(db.String(50), default='professional')
...
def resume_builder():
    ...
    if 'user_id' not in session:
        flash("Please log in to access the resume builder", "warning")
        return redirect(url_for('login'))
    
    user = User.query.get(session['user_id'])
    # Check if user has existing resume data
    existing_resume = ResumeData.query.filter_by(user_id=user.id, is_active=True).order_by(ResumeData.updated_at.desc()).first()
    
    return render_template('resume_builder.html', user=user, existing_resume=existing_resume)
...
def generate_summary():
    ...
    try:
        ...
        prompt = f"""Generate a professional summary for a resume with the following details:
...
Generate only the professional summary text, nothing else."""
...
def improve_description():
    ...
    try:
        ...
        prompt = f"""Improve this job responsibility for ATS optimization and impact:

Job Title: {job_title}
Original Description: {original_description}

Requirements:
- Make it action-oriented starting with strong action verbs
...
Provide only the improved description, nothing else."""
        ...
        return jsonify({'status': 'success', 'improved_description': improved})
        
    except Exception as e:
        print(f"Error improving description: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/check_ats', methods=['POST'])
def check_ats():
    ...
    try:
        ...
        if file.filename.endswith('.pdf'):
            try:
                # Try fitz (PyMuPDF) first as it's more robust for text extraction
                try:
                    import fitz
                    ...
                    if not resume_text.strip():
                        print("Fitz extracted no text, possibly a scanned PDF.")
                except Exception as e:
                    print(f"Fitz failed, falling back to PyPDF2: {e}")
                    file.seek(0)
                    import PyPDF2
                ...
                if not resume_text or not resume_text.strip():
                    return jsonify({'status': 'error', 'message': 'Could not extract text from PDF. It might be a scanned image or protected file. Please upload a machine-readable PDF or a .txt file.'}), 400
            except Exception as e:
                print(f"PDF extraction error: {e}")
                return jsonify({'status': 'error', 'message': f'Error reading PDF file: {str(e)}'}), 400
        elif file.filename.endswith('.txt'):
            resume_text = file.read().decode('utf-8')
        else:
            return jsonify({'status': 'error', 'message': 'Unsupported file format. Please upload PDF or TXT.'}), 400
...
1. Keyword Optimization: Rate how well keywords are used
2. Format Compatibility: Rate format ATS-friendliness
3. Section Completeness: Rate if all important sections are present
4. Contact Information: Rate if contact details are clear
5. Overall ATS Score: Overall rating (0-100)

Also provide 5-7 specific, actionable improvement suggestions.

Format your response as a valid JSON object only:
{{
  "scores": {{
    "keyword_optimization": <integer>,
    "format_compatibility": <integer>,
  ...
  "suggestions": [
    ...
    ...
        ...
        try:
            ...
            return jsonify({'status': 'error', 'message': 'AI returned an invalid format. Please try again.'}), 500
...
@app.route('/api/save_resume_data', methods=['POST'])
def save_resume_data():
    ...
    try:
        ...
        existing_resume = ResumeData.query.filter_by(user_id=user_id, is_active=True).first()
        
        if existing_resume:
            # Update existing resume
            existing_resume.personal_info = json.dumps(data.get('personal_info', {}))
            existing_resume.professional_summary = data.get('professional_summary', '')
            existing_resume.work_experience = json.dumps(data.get('work_experience', []))
            ...
            existing_resume.certifications = json.dumps(data.get('certifications', []))
            existing_resume.template_style = data.get('template_style', 'professional')
        ...
        db.session.commit()
        print(f"Successfully saved resume data for user {user_id}. Data size: {len(str(data))} bytes.")
        return jsonify({'status': 'success', 'message': 'Resume data saved successfully'})
...
@app.route('/api/get_resume_data', methods=['GET'])
def get_resume_data():
    ...
    try:
        user_id = session['user_id']
        resume = ResumeData.query.filter_by(user_id=user_id, is_active=True).order_by(ResumeData.updated_at.desc()).first()
        ...
        resume_data = {
            ...
            'template_style': resume.template_style or 'professional'
        }
